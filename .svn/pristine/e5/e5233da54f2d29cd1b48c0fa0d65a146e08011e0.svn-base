	...
	public static void send(String to,String code){
		...
		Authenticator authenticator = new Authenticator() {
			...
			public PasswordAuthentication getPasswordAuthentication() {
				 ...
				 * xxxxxxx:自己的QQ邮箱登录帐号，也就是qq号 yyyyyyy:密码，使用授权码登录，而不能使用原始的QQ密码
				 */
				return new PasswordAuthentication("juhe@zz624.com", "Ailin123");
			}
		};
		// 1、连接
		/**
		 * props 连接配置信息，邮件服务器的地址，是否进行权限验证 authenticator 权限验证，也就是帐号密码验证
		 * 所以需要先配置这两个参数
		 */
		...
		// 2.1、发件人是谁
		try {
			message.setFrom(new InternetAddress("juhe@zz624.com"));
		...
		/**
		 * 收件人是谁？ 第一个参数： RecipientType.TO 代表收件人 RecipientType.CC 抄送
		...
		try {
			...
			// 2.3 主题（标题）
			message.setSubject("JUHE聚合数据验证码");
			// 2.4 正文
			String str = "验证码： <br/>" +code+ "<br/>";
